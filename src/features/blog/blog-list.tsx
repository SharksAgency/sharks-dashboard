      ...
      <section className="surface overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] text-sm"><thead className="bg-[var(--surface-subtle)] text-xs text-[var(--muted)]"><tr><th className="px-4 py-3 text-start">العنوان</th><th className="px-4 py-3 text-start">التصنيف</th><th className="px-4 py-3 text-start">الحالة</th><th className="px-4 py-3 text-start">الكاتب</th><th className="px-4 py-3 text-start">التاريخ</th><th className="px-4 py-3 text-end">إجراءات</th></tr></thead><tbody className="divide-y">
            {filtered.map((post) => <tr key={post.id} className="hover:bg-[var(--surface-subtle)]"><td className="px-4 py-4"><strong className="block max-w-sm truncate">{post.title_ar}</strong><span className="text-xs text-[var(--muted)]" dir="ltr">/{post.slug}</span></td><td className="px-4 py-4">{post.category_id ? categoryMap.get(post.category_id) || "—" : "—"}</td><td className="px-4 py-4"><div className="flex gap-1"><Badge tone={post.status === "published" ? "success" : post.status === "archived" ? "warning" : "neutral"}>{post.status}</Badge>{post.is_featured ? <Badge tone="info">مميز</Badge> : null}</div></td><td className="px-4 py-4">{post.author_name}</td><td className="px-4 py-4 text-[var(--muted)]">{formatDate(post.published_at || post.updated_at)}</td><td className="px-4 py-4"><div className="flex justify-end gap-1"><a className="inline-flex size-9 items-center justify-center rounded-[6px] text-[var(--muted)] hover:bg-[var(--primary-soft)] hover:text-[var(--primary)]" href={`${process.env.NEXT_PUBLIC_PUBLIC_SITE_URL || "http://localhost:8443"}/blog/${post.slug}`} target="_blank" rel="noreferrer" title="Preview"><Icon name="external" size={17} /></a><Link className="inline-flex size-9 items-center justify-center rounded-[6px] text-[var(--muted)] hover:bg-[var(--surface-subtle)]" href={`/blog/${post.id}/edit`} title="Edit"><Icon name="edit" size={17} /></Link><Button variant="ghost" size="icon" onClick={() => run(post.id, "duplicate")} disabled={pending} title="Duplicate"><Icon name="copy" size={17} /></Button>{post.status === "published" ? <Button variant="ghost" size="sm" onClick={() => run(post.id, "unpublish")} disabled={pending}>إلغاء النشر</Button> : <Button variant="ghost" size="sm" onClick={() => run(post.id, "publish")} disabled={pending}>نشر</Button>}<Button variant="ghost" size="icon" className="hover:text-[var(--danger)]" onClick={() => setDeleting(post)}><Icon name="delete" size={17} /></Button></div></td></tr>)}
          </tbody></table>
